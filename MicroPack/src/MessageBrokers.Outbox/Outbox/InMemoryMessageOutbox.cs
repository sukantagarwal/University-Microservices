...
namespace MicroPack.MessageBrokers.Outbox.Outbox
{
    internal sealed class InMemoryMessageOutbox : IMessageOutbox, IMessageOutboxAccessor
    {
        ...
        private void RemoveExpiredMessages()
        {
            ...
            foreach (KeyValuePair<string, OutboxMessage> entry in _outboxMessages)
            {
                
                if (!entry.Value.ProcessedAt.HasValue)
                {
                    continue;
                }

                if (entry.Value.ProcessedAt.Value.AddSeconds(_expiry) > DateTime.UtcNow)
                {
                    continue;
                }
                
                _outboxMessages.TryRemove(entry.Key, out _);
                _inboxMessages.TryRemove(entry.Value.OriginatedMessageId, out _);
            }
        }
    }
}